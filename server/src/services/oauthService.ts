...
class OAuthService {
     ...
     */
    public async getTwitchAuthToken(
        authResponse: ITwitchRedirectResponse
    ): Promise<ITwitchUser> {
        return new Promise<ITwitchUser>(async (resolve, reject) => {
            ...
            try {
                ...
                if (
                    response.nonce.replace(new RegExp(" ", "g"), "+") ===
                    this.nonce
                ) {
                    const twitchAuth = await this.saveTwitchAuth(
                        response,
                        this.nonce
                    );
                    this.nonce = "";
                    ...
                    Logger.err(
                        LogType.OAuth,
                        `Nonce did not match. Expected: ${this.nonce} -- Got: ${response.nonce}`
                    );
                    this.nonce = "";
    ...
    public async refreshTwitchToken(username: string): Promise<string> {
        ...
        try {
            ...
            if (user.refreshToken && user.hasLogin) {
                const refreshToken = CryptoHelper.decryptString(
                    user.refreshToken
                );
                const options = {
                    method: "POST",
                ...
                const encryptedRefreshToken = CryptoHelper.encryptString(
     ...
     */
    private async saveTwitchAuth(
        twitchAuth: ITwitchAuthResponse,
        nonce: string
    ): Promise<ITwitchUser> {
        return new Promise<ITwitchUser>(async (resolve, reject) => {
            try {
                const idToken = await CryptoHelper.verifyTwitchJWT(
                    twitchAuth.id_token,
                    nonce
                );
                const encryptedRefreshToken = CryptoHelper.encryptString(
                ...
                if (user) {
                    ...
                    if (
                        idToken.preferred_username.toLowerCase() ===
                        "magentafall"
                    ) {
    ...
    private async getTwitchAccessToken(username: string): Promise<string> {
        let accessToken = this.cacheService.get(
            CacheType.OAuth,
            `${Constants.TwitchCacheAccessToken}.${username}`
        );
        if (!accessToken) {
